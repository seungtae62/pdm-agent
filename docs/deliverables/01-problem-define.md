...
- 기회 요인:
    ...
    - Edge 시스템이 이상 탐지 및 정량 분석을 담당하고, AI Agent가 원인 분석, 인사이트 추출, 자연어 기반 문서 생성을 담당하는 역할 분리 가능
...
| --- | --- | --- | --- |
| KPI 1: 이상 감지 → 정비 권고 처리시간 단축률 | 90% 이상 단축 | 이벤트 수신 ~ 작업지시서 생성까지 소요 시간 측정. 기존 수동 프로세스 대비 에이전트(수 분 이내) 비교 | 수동 프로세스 수 시간 (설비 확인 + 원인 분석 + 이력 조회 + 보고서 작성 + 작업지시서 작성) |
| KPI 2: Skills 도입 토큰 효율성 개선율 | 정상 이벤트 60% 절감 | Skills 도입 전(시스템 프롬프트에 전체 도메인 지식 상주: ~15K 토큰) 대비 도입 후(Core Skills 조건부 로딩: 정상 ~6K, 이상 ~12K) 토큰 사용량 비교 | MCP 기반 Tool 호출 시 프로토콜 오버헤드 + 시스템 프롬프트에 전체 지식 상주 (~15K 토큰) |
| KPI 3: Deep Search 도입 전후 분석 근거 확보 품질 | 교차 검증 가능 근거 수 3배 이상 증가, 다관점 커버리지 3개 관점 확보 | 도입 전(단일 에이전트 RAG 검색) 대비 도입 후(STORM 다관점 분석) 교차 검증 가능한 근거 수, 분석 응답의 다관점 커버리지 비교 | 단일 에이전트 RAG 검색 (단일 관점, 교차 검증 불가) |

## 핵심 기능 정의

1. **Edge/Agent 역할 분리**: Edge 시스템이 정량 연산(HI 산출, 추세 계산, RUL 예측 등)을 수행하고, Agent는 Edge 산출값을 도메인 지식에 기반하여 해석·판단. 수치 계산은 Edge, 맥락적 판단은 Agent로 명확히 분리

2. **Skills 기반 도메인 지식 관리**: 도메인 지식을 SKILL.md 파일로 모듈화하여 Core Skills(조건부 로딩)와 User Skills(개인화)로 계층 관리. 추론 단계에서 필요한 Skill만 선택적으로 로드하여 토큰 효율성 확보

3. **Self-Evolving Skills**: 분석 완료 후 실제 결과와의 차이를 감지하여 Core Skills를 자동 보정. 반복 분석을 통해 조직의 설비 운영 노하우가 Skill 파일로 자동 축적

4. **Deep Search Engine**: 복잡한 분석 요청 시 STORM 스타일 다관점 분석 엔진이 3개 고정 Research Agent(Maintenance Engineer, Senior Analyst, Equipment Specialist)를 병렬 운용. Critic의 Review-Revise 루프와 confidence score 기반 가중치 투표로 다각도 근거 확보

5. **E2E 자동화**: 이상 감지 이벤트 수신 → 결함 유형 식별 → 진행 단계 판정 → 열화 속도 평가 → RUL 맥락 평가 → 위험도 종합 판단 → 정비 권고 → 분석 리포트/작업지시서 생성까지 전 과정을 단일 에이전트가 자율적으로 수행
...
- 의사결정 품질 향상: 연산과 해석 역할 분리를 통해, 정량적 근거와 전문가 수준 판단이 결합된 신뢰성 있는 정비 의사결정 지원
