...
| RAG                          | 정비 이력 DB, 설비 매뉴얼 DB, 과거 분석 이력 DB에서 관련 정보를 검색하여 해석의 근거를 보강. 일반 RAG 활용(정보 조회)과 Deep Research(분석적 조사)에서 공통으로 사용되는 기반 기술 | VDB 기반. MCP Tool을 통해 호출되며, 검색 결과를 추론 컨텍스트에 반영                                                                              |
