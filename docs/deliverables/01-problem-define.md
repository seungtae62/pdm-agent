...
| Tool 호출 효율성    | 정성 평가     | 불필요한 Tool 호출 없이 상황에 적절한 Tool만 호출했는지 평가 (이벤트 분석 시 최소 RAG 활용, 대화형에서 사용자 요청 시 Deep Research 발동 등) | 미측정                                |
...
| Deep Research                | 단순 정보 조회(일반 RAG)를 넘어, 분석적 심층 조사를 수행하는 행동 패턴. 내부 RAG(정비 이력, 설비 매뉴얼, 분석 이력)와 외부 웹 검색을 조합하여 가설 수립 → 다중 소스 탐색 → 결과 해석 → 후속 질문 생성 → 재검색 → 종합 분석의 반복적 조사 루프를 자율 수행. 단순 RAG 검색 대비 심층적이고 구조화된 분석 결과를 도출 | 대화형 상호작용에서 사용자의 분석적 질문 시에만 발동. 이벤트 자동 분석에서는 미발동 (빠른 판정 + 알림 우선). 탐색 깊이와 반복 횟수는 에이전트가 자율 판단 |
...
| RAG                          | 정비 이력 DB, 설비 매뉴얼 DB, 과거 분석 이력 DB에서 관련 정보를 검색하여 해석의 근거를 보강. 일반 RAG 활용(정보 조회)과 Deep Research(분석적 조사)에서 공통으로 사용되는 기반 기술 | VDB 기반. MCP를 통해 호출되며, 검색 결과를 추론 컨텍스트에 반영                                                                                  |
