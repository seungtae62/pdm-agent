...
| MCP Tool 사용 규칙 | 상주 (시스템 프롬프트) | 5개 MCP Tool의 호출 조건 및 효율성 원칙 |
| 도메인 지식 | **온디맨드 (Agent Skills)** | fault-diagnosis, feature-interpret 등 SKILL.md로 분리. 추론 과정에서 필요 시에만 로드 |
| 위험도 기준 및 응답 양식 | **온디맨드 (Agent Skills)** | response-template Skill로 분리. 위험도 판정 후 로드 |
| Deep Research 절차 | **온디맨드 (Agent Skills)** | deep-research Skill로 분리. 대화형 상호작용에서 사용자 요청 시에만 로드 |
...
Agent의 능력은 **MCP(외부 실행)**와 **Agent Skills(도메인 지식)**의 이원 구조로 구성됩니다.

* **MCP Tools**: 외부 데이터 소스 및 알림 시스템과의 실제 상호작용을 담당합니다. 모든 Tool은 MCP Server로 구현되며, Agent가 MCP Client로 호출합니다.
* **Agent Skills**: 도메인 지식을 SKILL.md 파일로 모듈화하여, 필요 시에만 점진적으로 로드합니다. 시스템 프롬프트에는 최소한의 페르소나와 추론 구조만 유지합니다.
...
**MCP Tool 호출 원칙:**
* 정상 상태에서는 MCP Tool을 호출하지 않음 (Agent Skills로 로드된 도메인 지식으로 판단 완료)
...
| response-template | Thought 5 위험도 판정 후 로드 | Normal/Watch/Warning/Critical 위험도별 응답 양식, 리포트 구조, 작업지시서 포함 기준 |

**Skills 점진적 로딩 예시:**
* **정상 이벤트**: 시스템 프롬프트(페르소나 + 추론 구조) → Thought 1 조기 종료 → Skills 미로드 (토큰 절감)
* **이상 이벤트**: 시스템 프롬프트 → fault-diagnosis 로드 → feature-interpret 로드 → response-template 로드
...
| **전략** | **설명** | **적용 시점** |
| ------- | ------- | ---------- |
| Skills 점진적 로딩 | 도메인 지식을 Agent Skills로 분리. 세션 시작 시 메타데이터(~300토큰)만 로드하고, 추론 시 필요한 Skill만 온디맨드 로드(~5K토큰/Skill). 정상 이벤트에서는 결함 진단 Skill 미로드 | 매 턴 |
...
| **기술** | **구현** | **선정 사유** |
| ------- | ------- | ---------- |
| Tool 프로토콜 | MCP (Model Context Protocol) | LangGraph Agent가 MCP Client, Tool 서버가 MCP Server로 동작. 표준 프로토콜 기반 Tool 호출로 서버 독립 배포 및 확장 가능. 외부 시스템(Qdrant, 웹 검색, 알림)과의 인터페이스를 담당 |
