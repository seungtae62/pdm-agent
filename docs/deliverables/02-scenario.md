...
> 각 시점 개별 분석 (시점A: 5일차 정상 구간 / 시점B: 15일차 의심 전이 구간 / 시점C: 25일차 결함 진행 구간)
...
| 7 | - | ReAct Thought 3: 정비 권고의 구체성을 높이기 위해 Action Skills로 일반 RAG 활용. search_maintenance_history로 유사 내륜 결함 사례 검색, search_equipment_manual로 정비 절차 확인 | ReAct + Action Skills (RAG) | 정보 조회 목적의 선택적 Action Skills 호출 (1~2회) |
...
| 7 | - | Action Skills로 일반 RAG 활용: search_equipment_manual로 "외륜 결함 급속 열화 조건" 검색, search_maintenance_history로 "유사 급속 열화 사례" 검색 (1~2회 호출) | ReAct + Action Skills (RAG) | 정보 조회 목적의 선택적 Action Skills 호출 |
